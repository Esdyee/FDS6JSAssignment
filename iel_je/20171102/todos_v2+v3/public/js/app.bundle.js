...
var todos = [];
var todosFunc = function () {

  function getTodos(status) {
    _axios2.default.get('/todos').then(function (res) {
      todos = res.data;
      console.log(todos);
  ...
  var inputTextBox = document.getElementById('input-todo');
  ...
  function render(selTodos) {
    //todoList 초기화
    todoList.innerHTML = '';
    selTodos.forEach(function (todo) {
  ...
  inputTextBox.addEventListener('keydown', function (event) {
    if (event.keyCode === 13 && event.target.value !== '') {
      //max_id 검색
      var max_id = todos.length !== 0 ? Math.max.apply(Math, _toConsumableArray(todos.map(function (todo) {
        return todo.id;
      }))) + 1 : 1;
      ...
      var add_todo = {
        ...
        //추가
      };_axios2.default.post('/todos', add_todo).then(function (res) {/*console.log(res)*/}).catch(function (err) {
  ...
  todoList.addEventListener('click', function () {
    if (event.target.nodeName === "SPAN") {
      //삭제할 id span에서 검색
      var remove_id = event.target.getAttribute("data-id");
      //해당 id가 몇번째 인덱스에 있는가
      var remove_index = todos.find(function (todo) {
        return todo.id == remove_id;
      });

      //삭제할 인덱스의 요소 todos에서 삭제
      todos.splice(todos.indexOf(remove_index), 1);

      _axios2.default.delete('/todos/' + remove_id).then(function (res) {
  ...
  todoList.addEventListener('change', function () {
    if (event.target.nodeName === "INPUT") {
      ...
      _axios2.default.patch('/todos/' + event.target.id, toggle_todo).then(function (res) {
        return console.log(res);
      }).catch(function (err) {
        return console.log(err);
      });
  ...
  nav[0].addEventListener('click', function (event) {
    if (event.target.nodeName == "A") {
      ...
      for (var i = 0; i < nav_li_elmns.length; i++) {
  ...
  //All Select Check
  var allCheckbox = document.getElementById('chk-allComplete');
  allCheckbox.addEventListener('change', function (event) {
    _axios2.default.patch('/todos', { completed: event.target.checked }).then(function (res) {
      getTodos(true);
    }).catch(function (err) {
      return console.log(err);
    });
  ...
  //Count Todos
  function countTodos() {
    var spanCompleteCount = document.getElementById('completedTodos');
    var strongLeftCount = document.getElementById('leftTodos');

    var cmplTodos = todos.filter(function (todo) {
  ...
  //Clear Todos
  var btnClearCmpl = document.getElementById('btn-removeCompletedTodos');
  btnClearCmpl.addEventListener('click', function () {
    _axios2.default.delete('/todos/completed').then(function (res) {
      getTodos(true);
    }).catch(function (err) {
      return console.log(err);
    });
...
var normalizeHeaderName = __webpack_require__(341);
...
function getDefaultAdapter() {
  ...
  if (typeof XMLHttpRequest !== 'undefined') {
    ...
    adapter = __webpack_require__(333);
...
var createError = __webpack_require__(334);
var btoa = (typeof window !== 'undefined' && window.btoa && window.btoa.bind(window)) || __webpack_require__(347);
...
var utils = __webpack_require__(329);
var bind = __webpack_require__(331);
var Axios = __webpack_require__(340);
var defaults = __webpack_require__(330);
...
axios.CancelToken = __webpack_require__(354);
axios.isCancel = __webpack_require__(335);

// Expose all/spread
axios.all = function all(promises) {
  return Promise.all(promises);
};
axios.spread = __webpack_require__(355);
...
var isCancel = __webpack_require__(335);
var defaults = __webpack_require__(330);
var isAbsoluteURL = __webpack_require__(352);
var combineURLs = __webpack_require__(353);
